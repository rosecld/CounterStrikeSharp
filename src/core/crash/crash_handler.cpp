...
bool LooksLikeReturnAddress(uintptr_t address)
{
    ...
    if (FindModule(address) == nullptr && !InAnonExec(address)) return false;
...
void WriteFrameLocation(uintptr_t address)
{
    ...
    if (InAnonExec(address))
    {
        Out("anon-exec+0x");
        for (int index = 0; index < g_anonExecCount; ++index)
        {
            if (address >= g_anonExec[index].start && address < g_anonExec[index].end)
            {
                OutHex(address - g_anonExec[index].start, 0);
